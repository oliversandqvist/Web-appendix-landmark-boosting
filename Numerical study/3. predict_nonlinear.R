...
dataSurv <- readRDS("Results/dataSurv_nonlinear.rds")

tstop <- 1
h <- 1/100

coxNaive <- readRDS("Results/coxNaiveModels_nonlinear.rds")
coxStack <- readRDS("Results/coxStackModels_nonlinear.rds")
xgbNaive <- readRDS("Results/xgbNaiveModels_nonlinear.rds")
xgbStack <- readRDS("Results/xgbStackModels_nonlinear.rds")
...
for (i in 1:nrow(dataSurv)){
  ...
  for (modelName in names(xgbNaive)){
    ...
    X <- model.matrix(~ X1 + X2 + X3 + time, data = datai)
  ...
  for (modelName in names(xgbStack)){
    ...
    X <- model.matrix(~ X1 + X2 + X3 + S + time, data = datai)
    xgbDatai = xgb.DMatrix(X)
    setinfo(xgbDatai, "base_margin", log(datai$exposure)+log(F0))
    haz <- predict(object=fit, newdata=xgbDatai, type="response")
    surv <- exp(-sum(haz))
...
saveRDS(dataSurvPred, file = "Results/dataSurvPred_nonlinear.rds")
